...
export function LogoMark({ color = '#1B2D4F', size = 32, className = '' }: LogoProps & { size?: number }) {
  if (color === '#FFFFFF') {
    ...
    return <img src="/logo/footer-mark.PNG" alt="Rotation Analytics Inc" width={size} height={size} className={`object-contain ${className}`} />
  }
  // eslint-disable-next-line @next/next/no-img-element
  return <img src="/logo/rotation-mark.png" alt="Rotation Analytics Inc" width={size} height={size} className={`object-contain ${className}`} />
}
...
export function LogoMonogram({ color = '#1B2D4F', className = '' }: LogoProps) {
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Rotation Analytics Inc" className={className}>
      <title>Rotation Analytics Inc</title>
